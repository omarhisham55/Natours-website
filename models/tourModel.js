...
//: Responsible for embedding guides into tours
...
tourSchema.pre("aggregate", function (next) {
  ...
  const geoNearIndex = pipeline.findIndex((stage) => stage.$geoNear);

  if (geoNearIndex === 0) {
    pipeline.splice(1, 0, { $match: { secretTour: { $ne: true } } });
    pipeline.splice(2, 0, { $match: { description: { $ne: undefined } } });
  } else {
